...
const JobDetail = (props) => {
  const daysLeft = dayLeft(props.createdAt, props.end);
  return (
    <div className="text-blueGray-700 bg-white mt-0">
      <div className="container flex flex-col items-center px-2 py-10 mx-auto md:flex-row lg:px-28">
        <div className="flex flex-col items-start mb-16 text-left lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 md:mb-0">
          ...
          <h1 className="w-full break-words mb-2 text-2xl font-black tracking-tighter text-black  title-font">
            {props.title || "Job title"}
          </h1>
          <p className="w-full break-words mb-4 mb-8 text-base leading-relaxed text-left text-blueGray-600 ">
            {props.owner || "Job owner"}
          </p>
          <p className="w-full break-words mb-4 mb-8 text-base leading-relaxed text-left text-blueGray-600 ">
            Job description: {props.description || "Job description"}
          </p>
          <p className="w-full break-words mb-2 text-base leading-relaxed text-left text-blueGray-600 ">
            Prices: {props.price || "Job price"}$
          </p>
          <p className="w-full break-words  mb-2 text-base leading-relaxed text-left text-blueGray-600 ">
...
function dayLeft(createdAt=0, end=0) {
  let amount =(Date.now() - Date.parse(createdAt.toString()))/1000/3600/24
  let remain = end - amount
  if (remain > 1) {
